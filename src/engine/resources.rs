...
pub async fn load_texture(file_name: &str, device: &wgpu::Device, queue: &wgpu::Queue) -> Result<Texture> {
...
pub async fn load_model(
    ...
    for material in obj_materials? {
        ...
        let normal_texture = Rc::new(load_texture(&material.normal_texture, device, queue).await?);
    ...
    let meshes = models
        ...
        .map(|material| {
            ...
            let mut triangles = vec![0; vertices.len()];
            ...
            for (i, n) in triangles.into_iter().enumerate() {
